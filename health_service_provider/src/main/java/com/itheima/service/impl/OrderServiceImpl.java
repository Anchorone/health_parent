...
public class OrderServiceImpl implements OrderService {
    ...
    public Result order(Map map) throws Exception{
        ...
        if (member != null){
            ...
            member.setEmail((String)map.get("mail"));
        ...
        order.setOrderType((String)map.get("orderType"));//到诊类型
        order.setSetmealId(Integer.parseInt((String) map.get("setmealId")));//套餐ID
        orderDao.add(order);

        orderSetting.setReservations(orderSetting.getReservations() + 1);//已预约人数加一
        orderSettingDao.editReservationsByOrderDate(orderSetting);
        return new Result(true,MessageConstant.ORDER_SUCCESS,order.getId());
    }

    //根据预约ID查询相关信息（体检人姓名，预约日期，套餐名称，预约类型）
    @Override
